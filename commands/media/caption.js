...
export default {
	...
	async execute(message, args) {
		...
		try {
			...
			const maxWidth = Math.max(imageMetadata.width, 900);
			const fontSize = Math.max(maxWidth * 0.15, 60);
			...
			function wrapText(text, maxCharsPerLine) {
				...
				const effectiveMaxChars = Math.floor(maxWidth / (fontSize * 0.5));
                ...
                <defs>
                    <style>
                        @font-face {
                            ...
                            src: url('${path.resolve('./assets/fonts/Impact.ttf')}');
                        }
                    </style>
                </defs>
